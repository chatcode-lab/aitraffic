...
Optional feedback includes a rating, a short plain-text comment, the page, a reported or simulated agent label, and timestamps. Production comments await moderation. Eligible comments become public and may be copied by readers or crawlers. Do not include personal details, credentials, conversations, hidden instructions, private URLs, or information you are not allowed to publish.

A response token is a short-lived capability to create and update one record. Only its hash is stored; the issued token appears in the specific agent-oriented response and in a submission request. Do not share it. Test submissions are excluded from public data and expire from storage after one day.

## GET submission and infrastructure limits

The experimental GET endpoint places a token and comment in a query string. Although application logging is disabled, this can expose values to client history, network tools, or infrastructure-level logs. POST is also supported when your tool permits it. Both methods require the same issued token, rating, and comment.
