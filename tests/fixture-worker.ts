 ...
 async fetch(request:Request){
  if(new URL(request.url).pathname==='/fixture'){
   ...
   if(input.action==='reset'){
    ...
    for(const table of ['buckets','tokens','challenges','feedback','rate_limits'])this.ctx.storage.sql.exec(`DELETE FROM ${table}`);
...
export default {
 async fetch(request:Request,env:Env,ctx:ExecutionContext){
  ...
  if(request.headers.has('X-Fixture-No-Admin'))return worker.fetch(request,{...env,LAB_ADMIN_KEY:undefined},ctx);
